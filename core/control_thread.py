...
class DroneControlThread(QThread):
            ...
            while not self._stop_requested:
                ...
                while not self._stop_requested and not self._land_requested:
                    ...
                    if self.control_mode == "UDP自动控制":
                        ...
                        # 偏航角每2个周期更新（~100ms），为偏航P校正提供及时反馈
                        # 位置每10个周期更新（~500ms），位置P校正对实时性要求较低
    ...
    async def _process_udp(self, drone, cmd, data_recorder, pos):
        ...
        两阶段控制策略：
        - 第一包有效数据：闪现模式，用set_pose将无人机定位到飞控指定的经纬高+姿态
          同时用飞控经纬度更新home_geo_point，使NED坐标系原点对齐飞控位置
        - 后续包：速度跟踪模式，机体速度转NED + 自动校准 + P校正 + 偏航速率
        ...
        try:
            ...
            # 第一包有效位置数据：闪现到飞控指定位置 + 更新home_geo_point经纬度
            if not self._udp_pos_calibrated and (lat != 0 or lon != 0):
                await self._process_udp_first_packet(drone, cmd, data_recorder, pos, pkt)
            else:
    ...
    async def _process_udp_first_packet(self, drone, cmd, data_recorder, pos, pkt):
        ...
        # 闪现后强制刷新位置/GPS/偏航缓存，确保UI显示和后续控制用最新数据
        # set_pose不会自动更新_cached_pos等缓存，不刷新会导致GPS显示滞后
        try:
            loop = asyncio.get_event_loop()
            kin = await loop.run_in_executor(None, drone.get_ground_truth_kinematics)
            self._cached_pos = kin["pose"]["position"]
            orientation = kin.get("pose", {}).get("orientation", {})
            qw2 = orientation.get("w", 1.0)
            qx2 = orientation.get("x", 0.0)
            qy2 = orientation.get("y", 0.0)
            qz2 = orientation.get("z", 0.0)
            siny_cosp = 2 * (qw2 * qz2 + qx2 * qy2)
        ...
        # 位置校准：闪现后无人机已在目标位置，偏移量设为0
        # 注意：不能用pos计算偏移，因为pos是闪现前的旧位置，set_pose不会更新pos
        # 若用旧pos计算会得到巨大错误偏移，导致后续速度跟踪把无人机拉回旧位置
        self._udp_pos_offset_x = 0.0
        self._udp_pos_offset_y = 0.0
        self._udp_pos_offset_z = 0.0
        self._udp_pos_calibrated = True
        self._log(f"UDP首包闪现完成，位置校准偏移=(0.0, 0.0, 0.0)m", "INFO")
    ...
    async def _process_udp_track(self, drone, cmd, data_recorder, pos, pkt):
        ...
        # 偏航PD控制：前馈R + 角度PD校正
        # P项：角度误差驱动，快速纠正偏航偏差
        # D项：误差变化率阻尼，抑制过冲振荡，实现快速稳定
        # 前馈R：飞控角速率提供动态旋转响应
        # 归一化yaw_error到[-180°, 180°]，正确处理0°/360°环绕
        feedforward_yaw_rate = math.radians(R)
        psi = cmd.get("psi", 0)
        current_yaw_deg = math.degrees(self._cached_yaw_rad) % 360
        yaw_error = psi - current_yaw_deg
        if yaw_error > 180:
            yaw_error -= 360
        elif yaw_error < -180:
            yaw_error += 360
        ...
        Kp_yaw = 2.0
        Kd_yaw = 0.5
        d_error = yaw_error - self._prev_yaw_error
        self._prev_yaw_error = yaw_error
        correction_deg_per_s = Kp_yaw * yaw_error + Kd_yaw * d_error
        MAX_YAW_RATE = 90.0
        correction_deg_per_s = max(-MAX_YAW_RATE, min(MAX_YAW_RATE, correction_deg_per_s))

        # 前馈+PD校正合成偏航速率（不做EMA平滑，EMA引入相位滞后导致振荡）
        yaw_rate_rad = feedforward_yaw_rate + math.radians(correction_deg_per_s)
    ...
    def _setup_sensors(self, client, drone, data_recorder):
        ...
        # 传入control_thread作为优先数据源（含预转换的ue5_points），LidarCallback作为降级数据源
        lidar_cb = self._sensor_manager.get_sensor("lidar1")
        self._lidar_udp_sender = LidarUdpSender(lidar_cb, self._log, control_thread=self) if lidar_cb else None

        if "lidar1" in drone.sensors:
            lidar_topic = drone.sensors["lidar1"]["lidar"]
            def on_lidar(_, data):
                # 方案5+6：在回调线程一次性完成所有numpy解析和坐标转换
                # 回调线程做：numpy解析 + 零点过滤 + NED→OpenGL + NED→UE5
                # UI线程和LidarUdpSender直接使用预转换结果，零重复计算
                #
                # 坐标系说明：
                # ProjectAirSim的point_cloud已经是全局NED坐标（与官方LidarDisplay一致，
                # test_lidar_ls120s3.py直接使用point_cloud不做任何变换即可正确显示）。
                # 全局NED坐标已包含无人机位置信息，无需再做位置偏移。
                # 若加上无人机位置偏移会把点云推远，导致3D视图FOV被压缩（从120°缩到~90°）。
                #
                ...
                if data and "point_cloud" in data:
                    ...
                    if pc and len(pc) > 0 and len(pc) % 3 == 0:
                        try:
                            ...
                            # 零点过滤：一次性过滤无效点，后续所有消费者复用
                            mask = ~np.all(pts == 0, axis=1)
                            pts = pts[mask]
                            if len(pts) > 0:
                                # point_cloud已是全局NED坐标（含无人机位置），不做位置偏移
                                # 方案8：先从NED算ue5_pts，再从ue5_pts派生gl_pts，减少1次列赋值+取反运算
                                # NED→UE5坐标转换（UDP发送到310P用）
                                # NED: X=北, Y=东, Z=下 → UE5: X=前(北), Y=右(东), Z=上
                                ue5_pts = np.empty_like(pts)
                                ue5_pts[:, 0] = pts[:, 0]
                                ue5_pts[:, 1] = pts[:, 1]
                                ue5_pts[:, 2] = -pts[:, 2]
                                # NED→OpenGL坐标转换（UI 3D可视化用）
                                # 从ue5_pts派生：ue5(X=北,Y=东,Z=上) → OpenGL(X=东,Y=北,Z=上)
                                # gl_x=ue5_y=ned_y, gl_y=ue5_x=ned_x, gl_z=ue5_z=-ned_z
                                gl_pts = ue5_pts[:, [1, 0, 2]]
                                # 浅拷贝data避免修改原始对象，附加预转换坐标
                                data = dict(data)
                                data["gl_points"] = gl_pts
                                data["ue5_points"] = ue5_pts
                        except Exception:
                            pass
                with self._lidar_lock:
